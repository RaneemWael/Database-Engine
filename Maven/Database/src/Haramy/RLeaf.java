...
public class RLeaf extends RNode {
	...
	public RLeaf (int n, Boolean isRoot) {
		super (n, isRoot);
		if (isRoot) {
			this.minKeys = 1;
			this.minPointers = 1;
	...
	public void insert(Object indexKey, String pagePath, RTree t) {
		...
		for (int i=0; i<this.getValues().size(); i++) {
			...
			if (t.compareAll(values.get(i), indexKey) == 0) {
				...
				if (this.getPointers().get(i) instanceof ROverflowPage) {
					ROverflowPage overPage = (ROverflowPage) this.getPointers().get(i);
					overPage.insert(indexKey, pagePath, t);
					return;
				}
				else {
					String oldPagePath = (String) this.getPointers().remove(i);
					ROverflowPage overPage = new ROverflowPage (t.getN());
					this.getPointers().add(i, overPage);
					overPage.insert(this.getValues().get(i), oldPagePath, t);
					overPage.insert(indexKey, pagePath, t);
					return;
				}
			}
		...
		// 2) leaf overflow (add new one and shift if necessary)
			...
			if (this.isRoot) {
				...
				RLeaf newLeaf = new RLeaf (t.getN(), false);
				...
				if (this.prev != null) {
					...
					RLeaf prevLeaf = (RLeaf) this.getPrev();
	...
	public void delete (Object indexKey, String pagePath, RTree t) {
		...
		for (int i=0; i<this.getValues().size(); i++) {
			if (t.compareAll(values.get(i), indexKey) == 0) {
				if (this.getPointers().get(i) instanceof ROverflowPage) {
					ROverflowPage overPage = (ROverflowPage) this.getPointers().get(i);
					String pageLeft = overPage.delete(indexKey, pagePath);
					if (pageLeft != null) {
		...
		// 2) leaf can't handle deletion
			...
			if (this.getNext() != null)
				nextNode = (RLeaf) (this.getNext());
			
			if (this.getPrev() != null)
				prevNode = (RLeaf) (this.getPrev());
