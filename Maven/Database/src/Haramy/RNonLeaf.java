...
public class RNonLeaf extends RNode {
	
	public RNonLeaf(int n, Boolean isRoot) {
		super (n, isRoot);
		if (isRoot) {
			this.minKeys = 1;
			this.minPointers = 1;
	...
	public void replace (Object indexKey1, Object indexKey2, RTree t) {
		...
		if (this.isRoot) {
			...
			RNonLeaf nextNode = (RNonLeaf) (this.getParent());
			nextNode.replace(indexKey1, indexKey2, t);	
		}
	}
	
	public void insert (Object indexKey, RNode nodePath, RTree t) {
		...
		// 3) non-leaf overflow (add new one and shift if necessary)
			...
			if (this.isRoot) {
				...
				RNonLeaf newNode = new RNonLeaf (t.getN(), false);
				newNode.setNext(this);
				this.setPrev(newNode);
				
				RNode fixParent = (RNode) (this.getPointers().get(0));
				...
				while (j < this.minKeys) {
					...
					fixParent = (RNode) (this.getPointers().get(0));
					fixParent.setParent(newNode);
					newNode.getPointers().add(this.getPointers().remove(0));
					j++;
				}
				...
				RNonLeaf newRoot = new RNonLeaf (t.getN(), true);
				...
				RNonLeaf parentNode = (RNonLeaf) (this.getParent());
				...
				if (this.prev != null) {
					...
					RNonLeaf prevLeaf = (RNonLeaf) (this.getPrev());
	...
	public void delete (Object indexKey, boolean isNegative, RTree t) {
		...
		if (this.isRoot) {
			...
			for (int i=0; i<this.getValues().size(); i++) {
				if (t.compareAll(values.get(i), indexKey) == 0) {
					...
					if (this.getPointers().size() == 1) {
						RNode newRoot = (RNode) (this.getPointers().get(0));
			...
			// 2) node can't handle deletion
				...
				RNonLeaf nextNode = null;
				RNonLeaf prevNode = null;
				
				if (this.getNext() != null)
					nextNode = (RNonLeaf) (this.getNext());
				
				if (this.getPrev() != null)
					prevNode = (RNonLeaf) (this.getPrev());
				...
				if ((nextNode != null) && (nextNode.getParent()).equals(this.getParent()) && nextNode.getValues().size()-1 >= nextNode.getMinKeys()) {
					...
					RNode borrowedNode = (RNode) nextNode.getPointers().get(0);
					...
					Object lowValueAfterRemove = ((RNode) nextNode.getPointers().get(1)).lowestValue();
					
					RNonLeaf nextParent = (RNonLeaf) (nextNode.getParent());
				...
				//borrow left
					...
					RNode borrowedNode = (RNode) prevNode.getPointers().get(prevNode.getPointers().size()-1);
					borrowedNode.setParent(this);
				...
				//merge
					...
					if ((prevNode != null) && (prevNode.getParent()).equals(this.getParent()) && (prevNode.getValues().size() + (this.getValues().size()-1)) <= prevNode.getMaxKeys()) {
						...
						for (int i=0; i<this.getValues().size(); i++) {
							RNode move = (RNode) (this.getPointers().get(0));
					...
					//merge with right
						...
						for (int i=this.getValues().size()-1; i>=0; i--) {
							RNode move = (RNode) (this.getPointers().get(i));
